  ...
  ngOnInit(): void {
    ...
    this.realEstateSer.getAllRealEstateRequests().subscribe((res) => {
      ...
      this.singleFileStaticData = [
        ...
        {
          title: 'صوره ضهر البطاقه',
          image: `./assets/idback2.jpg`,
        },
        {
          title: 'صوره وجه البطاقه للوكيل',
          image: `./assets/idfront2.jpg`,
        },
        {
          title: 'صوره ضهر البطاقه للوكيل',
          image: `./assets/idback.jpg`,
        },
      ];
      this.multipleFileStaticData = [
        {
          title: 'صور محل الطلب',
          images: this.userData.requestImage.map(
            (ele: File) => `./assets/فيلا للبيع.jpg`
          ),
        },
        {
          title: 'صور سند الملكيه',
          images: this.userData.propertyImage.map(
            (ele: File) => `./assets/سند ملكية.jpg`
          ),
        },
        {
          title: 'صور التوكيل',
          images: this.userData.agentImage.map(
            (ele: File) => `./assets/${ele.name}`
          ),
        },
      ];
    });
